...
class EGNN_predictor(nn.Module):
    ...
    def forward(self, xh, node_mask, edge_mask, t=torch.zeros(1)):
        ...
        edge_attr = torch.sum((x[edges[0]] - x[edges[1]]) ** 2, dim=1, keepdim=True)
        h_final, h_embed, _ = self.egnn(
            h, x, edges, node_mask=node_mask, edge_mask=edge_mask, edge_attr=edge_attr
        )

        # print(h_final.shape, h_embed.shape)

        h_final = h_final.view(bs, n_nodes, -1)
        h_embed = h_embed.view(bs, n_nodes, -1)

        # print(h_final.shape, h_embed.shape, bs, n_nodes)

        return h_final.mean(1), h_embed.mean(1)
