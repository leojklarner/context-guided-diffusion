...
def quantize_mol(adjs):                
    ...
    print("in quantize_mol", torch.unique(adjs), adjs.dtype)
    ...
    print("after quant in quantize_mol", torch.unique(adjs), adjs.dtype)
    print("after int 64 cast", torch.unique(adjs.to(torch.int64)), adjs.to(torch.int64).dtype)
